...
/**
 * These test cases can be used to test-drive a solution to the diamond kata, in
 * an incremental manner.
 *
 * Instructions: 1. Make the first test case for Diamond A pass 2. remove the
 * '@Ignore' and enable the next test case. 3. Uncomment the one line of the
 * test case. Make it pass. Repeat until all lines are uncommented 4. When all
 * the lines of code in the test case are passing, enable the next test case. 5.
 * When all the test cases in this file are uncommented and passing, you should
 * have a full working solution.
 */
public class DiamondIncrementalTest {
	...
	public void LetterSequence_is_list_of_letters_on_each_line_of_the_diamond() {
		...
		assertCharacterSequence(new Diamond('B').getLetterSequence(), 'A', 'B',
				'A');
		assertCharacterSequence(new Diamond('C').getLetterSequence(), 'A', 'B',
				'C', 'B', 'A');
		assertCharacterSequence(new Diamond('D').getLetterSequence(), 'A', 'B',
				'C', 'D', 'C', 'B', 'A');
	}
	...
	public void IndentationSequence_is_a_list_of_indents_for_each_line_of_the_diamond() {
		...
		assertIntegerSequence(new Diamond('B').getIndentationSequence(), 1, 0,
				1);
		assertIntegerSequence(new Diamond('C').getIndentationSequence(), 2, 1,
				0, 1, 2);
		assertIntegerSequence(new Diamond('D').getIndentationSequence(), 3, 2,
				1, 0, 1, 2, 3);
	}
	...
	public void BetweenSequence_is_a_list_of_how_many_spaces_there_are_between_repeated_letters_on_each_line_of_the_diamond() {
		...
		assertIntegerSequence(new Diamond('C').getBetweenSequence(), 0, 1, 3,
				1, 0);
		assertIntegerSequence(new Diamond('D').getBetweenSequence(), 0, 1, 3,
				5, 3, 1, 0);
	}
	...
	public void OneRow_is_a_list_of_chars_in_one_diamond_row() {
		...
		assertCharacterSequence(new Diamond('B').getOneRow('B', 0, 1), 'B',
				' ', 'B');
		assertCharacterSequence(new Diamond('D').getOneRow('C', 1, 3), ' ',
				'C', ' ', ' ', ' ', 'C');
	}
	...
	public void assertCharacterSequence(List<Character> sequence,
			Character... expectedChars) {
		assertArrayEquals(expectedChars, sequence.toArray());
	}

	public void assertIntegerSequence(List<Integer> sequence,
